...
public class JUnit5 implements TestEngine {
    private static final Logger LOGGER = Logging.getLogger(JigsawPlugin.class);
    private static final String JUNIT_MODULE_NAME = "org.junit.jupiter.api";
    ...
    public boolean accepts(Project project) {
        ...
        for(Dependency dependency: configuration.getDependencies()) {
            if (dependency.getGroup().equals("org.junit.jupiter") && dependency.getName().equals("junit-jupiter-api")) {
                LOGGER.debug("Found JUnit 5 dependency - using JUnit 5 Jigsaw test engine configuration");
                return true;
            }
        }
        return false;
